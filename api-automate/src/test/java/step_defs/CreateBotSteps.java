...
public class CreateBotSteps {
	...
	public void base_url_as_https_automate_io_logs_generated_in_file(String host, String logFile) {
		...
		baseURL = host;
		csrf = "41GPycMcFGZdDSnHibPBCxyz";
		xsrf = "HnM8Z2RG-ZzD6MNMiDszpFNmL20hAIsBRpkY";
	...
	public void hit_post_method_for_login_with_arguments_email_and_password(String email, String password) {
		Header reqHeader = new Header("x-xsrf-token", xsrf);
		...
		response = given().cookie("_csrf", csrf).contentType(ContentType.JSON).accept(ContentType.JSON).header(reqHeader).header(new Header("_csrf", csrf))
				.body("{\"email\":\"" + email + "\",\"password\":\"" + password + "\"}").cookie("_csrf", csrf).cookie("x-xsrf-token",xsrf).when()
	...
	public void hit_post_method_for_Creating_Bot_to_forward_new_email_to(String toEmail) throws IOException {
		...
		Header reqHeader1 = new Header("User-Agent",
				"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.130 Safari/537.36");
		Header reqHeader2 = new Header("x-xsrf-token", xsrf);
		Headers headers = new Headers(reqHeader1,reqHeader2);
		logger.info("Login URI: "+baseURL + APIResourceURI.WORKFLOW_URI);
		logger.info("Request Payload: \n"+jsonBody);
		logger.info("Content-Type: "+ContentType.JSON);
		logger.info("Request Cookie: {_csrf : "+csrf+" }");
		logger.info("Request Cookie: {connect.sid : "+cookie+" }");
		logger.info("Accept: "+ContentType.JSON);
		logger.info("Headers: {"+reqHeader1.getName()+" : "+reqHeader1.getValue()+", "+reqHeader2.getName()+ " : "+reqHeader2.getValue()+" }");
		response = given().cookie("connect.sid", cookie).cookie("_csrf",csrf).cookie("x-xsrf-token",xsrf).contentType(ContentType.JSON)
				.accept(ContentType.JSON).headers(headers).body(jsonBody).when()
	...
	public void response_code_should_be_ok_and_status_will_be_success_and_message_as_Bot_successfully_stored(
		...
		if(data == null){
			data = response.jsonPath().getMap("error");
		}
		workflow_id = data.get("_id");
		String actual_message = data.get("message");
		logger.assertLog(code==actual_code, "Response status code mismatch. Expected: "+code+" Actual: "+actual_code);
		logger.assertLog(status.equals(actual_status), "Response status mismatch. Expected: "+status+" Actual: "+actual_status);
	...
	public void get_method_to_enable_bot_we_created_in_previous_step() {
		...
		response = given().cookie("connect.sid", cookie).cookie("_csrf", csrf).cookie("x-xsrf-token",xsrf).contentType(ContentType.JSON)
				.accept(ContentType.JSON).headers(headers).queryParam("skipPremiumCheck", true).when()
	...
	public void logout_the_session_and_verify_message_Session_expired(int code, String status, String message) {
		...
		logger.info("Accept: "+ContentType.JSON);
		logger.info("Headers: {"+reqHeader1.getName()+" : "+reqHeader1.getValue()+", "+reqHeader2.getName()+ " : "+reqHeader2.getValue()+" }");
		response = given().cookie("connect.sid", cookie).cookie("_csrf", csrf).contentType(ContentType.JSON).cookie("x-xsrf-token",xsrf).header(new Header("-csrf", csrf))
				.accept(ContentType.JSON).headers(headers).when().delete(baseURL + APIResourceURI.LOGOUT_URI);
		int actual_code=response.getStatusCode();
		String actual_status=response.jsonPath().get("status");
		Map<String, String> data = response.jsonPath().getMap("data");
		if(data == null){
